...
export default function ConventionUpdateForm(props) {
  const {
    id: idProp,
    convention: conventionModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = conventionRecord
      ? {
          ...initialValues,
          ...conventionRecord,
          Teilnehmers: linkedTeilnehmers,
        }
      : initialValues;
    setName(cleanValues.Name);
    setLocation(cleanValues.Location);
    setTeilnehmerobergrenze(cleanValues.Teilnehmerobergrenze);
    setMindestalter(cleanValues.Mindestalter);
    setTeilnehmers(cleanValues.Teilnehmers ?? []);
    setCurrentTeilnehmersValue(undefined);
    setCurrentTeilnehmersDisplayValue("");
    setErrors({});
  };
  const [conventionRecord, setConventionRecord] =
    React.useState(conventionModelProp);
  const [linkedTeilnehmers, setLinkedTeilnehmers] = React.useState([]);
  const canUnlinkTeilnehmers = false;
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Convention, idProp)
        : conventionModelProp;
      setConventionRecord(record);
      const linkedTeilnehmers = record
        ? await Promise.all(
            (
              await record.Teilnehmers.toArray()
            ).map((r) => {
              return r.teilnehmer;
            })
          )
        : [];
      setLinkedTeilnehmers(linkedTeilnehmers);
    };
    queryData();
  }, [idProp, conventionModelProp]);
  React.useEffect(resetStateValues, [conventionRecord, linkedTeilnehmers]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const promises = [];
          const teilnehmersToLinkMap = new Map();
          const teilnehmersToUnLinkMap = new Map();
          const teilnehmersMap = new Map();
          const linkedTeilnehmersMap = new Map();
          Teilnehmers.forEach((r) => {
            const count = teilnehmersMap.get(getIDValue.Teilnehmers?.(r));
            const newCount = count ? count + 1 : 1;
            teilnehmersMap.set(getIDValue.Teilnehmers?.(r), newCount);
          });
          linkedTeilnehmers.forEach((r) => {
            const count = linkedTeilnehmersMap.get(getIDValue.Teilnehmers?.(r));
            const newCount = count ? count + 1 : 1;
            linkedTeilnehmersMap.set(getIDValue.Teilnehmers?.(r), newCount);
          });
          linkedTeilnehmersMap.forEach((count, id) => {
            const newCount = teilnehmersMap.get(id);
            if (newCount) {
              const diffCount = count - newCount;
              if (diffCount > 0) {
                teilnehmersToUnLinkMap.set(id, diffCount);
              }
            } else {
              teilnehmersToUnLinkMap.set(id, count);
            }
          });
          teilnehmersMap.forEach((count, id) => {
            const originalCount = linkedTeilnehmersMap.get(id);
            if (originalCount) {
              const diffCount = count - originalCount;
              if (diffCount > 0) {
                teilnehmersToLinkMap.set(id, diffCount);
              }
            } else {
              teilnehmersToLinkMap.set(id, count);
            }
          });
          teilnehmersToUnLinkMap.forEach(async (count, id) => {
            const conventionTeilnehmerRecords = await DataStore.query(
              ...
              (r) =>
                r.and((r) => {
                  ...
                  return [
                    r.teilnehmerId.eq(recordKeys.id),
                    r.conventionId.eq(conventionRecord.id),
                  ];
                })
            );
          ...
          teilnehmersToLinkMap.forEach((count, id) => {
            for (let i = count; i > 0; i--) {
              promises.push(
                DataStore.save(
                  new ConventionTeilnehmer({
                    convention: conventionRecord,
                    teilnehmer: teilnehmerRecords.find((r) =>
          ...
          promises.push(
            DataStore.save(
              Convention.copyOf(conventionRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ConventionUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || conventionModelProp)}
